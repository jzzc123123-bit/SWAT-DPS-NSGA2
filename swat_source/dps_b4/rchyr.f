...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real*8 :: dps_tnkg, dps_qvol, dps_cout
      real*8 :: dps_tnkg2, dps_qvol2, dps_cout2      
      real*8 :: dps_tnkg3, dps_qvol3, dps_cout3
      real*8 :: dps_tnkg4, dps_qvol4, dps_cout4
      idlast = 0
      idlast = i - (id1 - 1)
 ...
 !! added for Total P (org P + sol p outs)to output.rch gsm 10/17/2011
        ...
        !===== DPS state extraction START =====
        !第一组
        if (dps_ready == 1) then
          if (dps_target_sub == j) then

            dps_state_reach = j
              
            dps_tnkg = pdvar(43)

!           pdvar(2) 是年平均流量(m3/s)，乘全年秒数变成年体积(m3)
            dps_qvol = pdvar(2) * 86400.d0 * dfloat(idlast)

            dps_tn_out_mass_yr(curyr) = dps_tnkg
            dps_q_out_vol_yr(curyr)   = dps_qvol

            dps_tn_allout_mass_yr(curyr) = dps_tnkg
            dps_q_allout_vol_yr(curyr)   = dps_qvol
            
            if (dps_qvol > 1.d-12) then
              dps_cout = 1000.d0 * dps_tnkg / dps_qvol
            else
              dps_cout = 0.d0
            end if

            dps_allctn_out_yr(curyr) = dps_cout

            if (dps_cstd > 1.d-12) then
              dps_z_yr(curyr,1) =
     &          min(dps_cout / dps_cstd, dps_smax) / dps_smax
            else
              dps_z_yr(curyr,1) = 0.d0
            end if

            if (dps_debug == 1) then
              write(*,*) 'RCHYR state: year=', curyr,
     &                   ' sub=', j,
     &                   ' TNload=', dps_tnkg,
     &                   ' Qvol=', dps_qvol,
     &                   ' TNconc=', dps_cout,
...
!           pdvar(2) 是年平均流量(m3/s)，乘全年秒数变成年体积(m3)
            dps_qvol2 = pdvar(2) * 86400.d0 * dfloat(idlast)

            dps_tn_out_mass_yr(curyr) = dps_tnkg2
            dps_q_out_vol_yr(curyr)   = dps_qvol2

            if (dps_qvol > 1.d-12) then
              dps_cout2 = 1000.d0 * dps_tnkg2 / dps_qvol2
            else
              dps_cout2 = 0.d0
            end if

            dps_ctn_out_yr(curyr) = dps_cout

            if (dps_cstd > 1.d-12) then
              dps_z_yr(curyr,2) =
     &          min(dps_cout2 / dps_cstd, dps_smax) / dps_smax
            else
              dps_z_yr(curyr,2) = 0.d0
            end if
...
!           pdvar(2) 是年平均流量(m3/s)，乘全年秒数变成年体积(m3)
            dps_qvol3 = pdvar(2) * 86400.d0 * dfloat(idlast)

            dps_tn_out_mass_yr(curyr) = dps_tnkg3
            dps_q_out_vol_yr(curyr)   = dps_qvol3

            if (dps_qvol > 1.d-12) then
              dps_cout3 = 1000.d0 * dps_tnkg3 / dps_qvol3
            else
              dps_cout3 = 0.d0
            end if

            dps_ctn_out_yr(curyr) = dps_cout3

            if (dps_cstd > 1.d-12) then
              dps_z_yr(curyr,3) =
     &          min(dps_cout3 / dps_cstd, dps_smax) / dps_smax
            else
              dps_z_yr(curyr,3) = 0.d0
            end if
...
!           pdvar(2) 是年平均流量(m3/s)，乘全年秒数变成年体积(m3)
            dps_qvol4 = pdvar(2) * 86400.d0 * dfloat(idlast)

            dps_tn_out_mass_yr(curyr) = dps_tnkg4
            dps_q_out_vol_yr(curyr)   = dps_qvol4

            if (dps_qvol > 1.d-12) then
              dps_cout4 = 1000.d0 * dps_tnkg4 / dps_qvol4
            else
              dps_cout4 = 0.d0
            end if

            dps_ctn_out_yr(curyr) = dps_cout4

            if (dps_cstd > 1.d-12) then
              dps_z_yr(curyr,4) =
     &          min(dps_cout4 / dps_cstd, dps_smax) / dps_smax
            else
              dps_z_yr(curyr,4) = 0.d0
            end if
