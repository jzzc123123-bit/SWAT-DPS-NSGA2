...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real*8 :: sum, fac_target, days_next
      real*8 :: lorg_cur, lno3_cur, lnh3_cur,lorg_cur2, lno3_cur2, lnh3_cur2,lorg_cur3, lno3_cur3, lnh3_cur3
      real*8 :: lorg_cur4, lno3_cur4, lnh3_cur4
      real*8 :: sum_org, sum_no3, sum_nh3,sum_org2, sum_no32, sum_nh32,sum_org3, sum_no33, sum_nh33
...
!     先算每个 RBF 基函数响应
            do irbf = 1, dps_nrbf
                 ...
                 do jdim = 1, dps_bdim
                    dist2_rbf = dist2_rbf +
     &             ((dps_z_yr(curyr,jdim) - dps_c(irbf,jdim))**2) /
     &               (dps_r(irbf,jdim)**2)
                 end do
...
!           1) 根据当前年状态计算下一年策略
      ...
      !      end do
            do kout = 1, dps_kout
                 ...
                 do irbf = 1, dps_nrbf
                        dps_u_raw_yr(curyr+1,kout) =
     &                    dps_u_raw_yr(curyr+1,kout) +
     &                        dps_w(irbf,kout) * phi_rbf(irbf)
                 end do
            end do

            do kout = 1, dps_kout
            dps_u_yr(curyr+1,kout) = min(1.d0,
     &                          max(0.d0,dps_u_raw_yr(curyr+1,kout)))
            end do
      
            do kout = 1, dps_kout
            dps_fac_yr(curyr+1,kout) = 0.4d0 + 0.8d0 * 
...
!===== write DPS evaluation file =====
     ...
     &           action='write')
            write(991,'(A)') 'year TNload TNconc u1 u2 u3 fac1 fac2 fac3'
          else
     ...
     &         dps_tn_allout_mass_yr(curyr),
     &         dps_allctn_out_yr(curyr),
     &         dps_u_yr(curyr,1),
     &         dps_u_yr(curyr,2),
     &         dps_u_yr(curyr,3),
     &         dps_fac_yr(curyr,1),
