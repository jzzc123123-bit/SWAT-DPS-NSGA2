...
# 0) 基础配置
# =========================================================
BASE_TEMPLATE_DIR = r"E:\Heihe\panlongjiangPOINTsourceDPS30yr\Scenarios\Default\TxtInOutB=4"
RUN_ROOT = r"E:\Heihe\panlongjiangPOINTsourceDPS30yr\Scenarios\Default\B=4_parallel_runs"
EXE_NAME = "SWATDPS_v2.2.exe"
...
# ***** 读取顺序严格按下列顺序假设 *****
# 1) a(k), k=1..3
# 2) w(irbf,kout), irbf=1..8, kout=1..3
# 3) c(irbf,jdim), irbf=1..8, jdim=1..4
# 4) r(irbf,jdim), irbf=1..8, jdim=1..4
# 5) 最后 Python 自动追加 job_id
# =========================================================
DPS_N_RBF = 8
DPS_KOUT = 3
DPS_BDIM = 4
...
N_VAR = 91
N_CMD_ARGS_TOTAL = 92   # 91变量 + 1个job_id


def build_bounds_and_cols_92():
    xl = []
    xu = []
    cols = []

    # a(k), k = 1..3
    for kout in range(1, DPS_KOUT + 1):
        xl.append(0.0)
        xu.append(1.0)
        cols.append(f"a{kout}")

    # w(irbf,kout), irbf 外层, kout 内层
    for irbf in range(1, DPS_N_RBF + 1):
        for kout in range(1, DPS_KOUT + 1):
            xl.append(-1.0)
            xu.append(1.0)
            cols.append(f"w{irbf}{kout}")

    # c(irbf,jdim), irbf 外层, jdim 内层
    for irbf in range(1, DPS_N_RBF + 1):
        for jdim in range(1, DPS_BDIM + 1):
            xl.append(0.0)
            xu.append(1.0)
            cols.append(f"c{irbf}{jdim}")

    # r(irbf,jdim), irbf 外层, jdim 内层
    for irbf in range(1, DPS_N_RBF + 1):
        for jdim in range(1, DPS_BDIM + 1):
            xl.append(0.01)
            xu.append(1.0)
            cols.append(f"r{irbf}{jdim}")

    return np.array(xl, dtype=float), np.array(xu, dtype=float), cols


XL, XU, X_COLS = build_bounds_and_cols_92()
...
def read_dps_eval(workdir: str, job_id: int) -> pd.DataFrame:
    ...
    if ENABLE_EVAL_YEAR_CHECK:
        if len(df) != EXPECTED_N_YEARS:
            raise ValueError(
                f"dps_eval 行数异常：期望 {EXPECTED_N_YEARS} 行，实际 {len(df)} 行"
            )
...
# Python 参数顺序必须与 Fortran 完全一致：
# [a1,a2,a3,
#  w11,w12,w13,w21,w22,w23,...,w81,w82,w83,
#  c11,c12,c13,c14,c21,c22,c23,c24,...,c84,
#  r11,r12,r13,r14,r21,r22,r23,r24,...,r84]
# 最后自动追加 job_id
# =========================================================
def run_swat_one_candidate(x: np.ndarray) -> tuple[int, pd.DataFrame]:
    ...
    try:
        ...
        if x.size != N_VAR:
            raise ValueError(f"输入参数长度={x.size}，但 92 参数版要求 N_VAR={N_VAR}")
...
def save_convergence_outputs(hist_df: pd.DataFrame, out_dir: str, tail_n: int = 10):
    ...
    hist_csv = os.path.join(out_dir, "nsga2_convergence_history_92.csv")
    ...
    summary_csv = os.path.join(out_dir, "nsga2_convergence_summary_92.csv")
...
def main():
    print("\n================ 当前 DPS 版本 ================")
    print("说明              = 91个决策变量（总命令行参数=92，最后一个是 job_id）")
    print("假设结构          = a(1:3), w(1:8,1:3), c(1:8,1:4), r(1:8,1:4), job_id")
    print("命令行读取顺序     = a -> w -> c -> r -> job_id")
    print(f"N_VAR             = {N_VAR}")
    print(f"N_CMD_ARGS(total) = {N_CMD_ARGS_TOTAL}")
    print(f"N_RBF             = {DPS_N_RBF}")
    print(f"KOUT              = {DPS_KOUT}")
    print(f"BDIM              = {DPS_BDIM}")
    print("==============================================\n")
    ...
    try:
        ...
        if res.X is None or res.F is None:
            raise RuntimeError(
                ...
                "2) Fortran 是否仍然接收 91 个变量 + 1 个 job_id；"
                "3) Python 与 Fortran 的参数顺序是否完全一致；"
                "4) CMAX / EXCEED_FRAC_MAX 是否设得过严；"
                "5) CSTD/CMAX 是否与你当前研究设定一致。"
            )
        ...
        out_csv = os.path.join(BASE_TEMPLATE_DIR, "pareto_solutions_nsga2_92param.csv")
        df_out.to_csv(out_csv, index=False, encoding="utf-8-sig")

        cfg_json = os.path.join(BASE_TEMPLATE_DIR, "nsga2_run_config_92param.json")
        with open(cfg_json, "w", encoding="utf-8") as f:
            json.dump({
                "desc": "91个决策变量（总命令行参数=92，最后一个是 job_id）",
                "assumption": "采用 a(1:3), w(1:8,1:3), c(1:8,1:4), r(1:8,1:4), job_id",
                "read_order_note": "Python与Fortran统一为 a -> w -> c -> r -> job_id",
                "n_rbf": DPS_N_RBF,
                "n_kout": DPS_KOUT,
                "n_bdim": DPS_BDIM,
                "n_var": N_VAR,
                "n_cmd_args_total": N_CMD_ARGS_TOTAL,
                "x_cols": X_COLS,
                "base_template_dir": BASE_TEMPLATE_DIR,
                "run_root": RUN_ROOT,
                "exe_name": EXE_NAME,
                "dps_eval_columns": ["year", "TNload", "TNconc", "u1", "u2", "u3", "fac1", "fac2", "fac3"],
                "dps_eval_note": "当前 dps_eval 中同一行 TNload/TNconc/u/fac 已对齐，且无 z 列",
                "eval_year_check_enabled": ENABLE_EVAL_YEAR_CHECK,
                "eval_year_range": [EVAL_YEAR_START, EVAL_YEAR_END],
                "cstd": CSTD,
